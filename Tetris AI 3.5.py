    ...
    if spawnNewItem == True:
        ...
        for i in range(len(shapeToSpawn[0])):
            if myGrid[shapeToSpawn[0][i][1]][shapeToSpawn[0][i][0]] == 0:
                ...
                if playerScore > 0:
                    textFile = open("AI Score List 5.txt","a")
...
def nextSpace(line):
    ...
    if leftMoves == 10:
        if len(possibleAreasToPlace) > 0:
            ...
            possibleAreasToPlace = sorted(sorted(possibleAreasToPlace, key = lambda x : x[1]), key = lambda x : x[4], reverse = True)  
            #print(b)  
            #[('Abel', 3), ('Al', 2), ('Carol', 2), ('Zeke', 2), ('Bill', 1), ('Chris', 1)]
            ...
            for i in range(possibleAreasToPlace[0][0]):
                moveRight()
            #print("Gaps = " + str(possibleAreasToPlace[0][1] - possibleAreasToPlace[0][4]))
