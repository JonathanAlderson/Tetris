    ...
    if spawnNewItem == True:
        ...
        for i in range(len(shapeToSpawn[0])):
            if myGrid[shapeToSpawn[0][i][1]][shapeToSpawn[0][i][0]] == 0:
                ...
                if playerScore > 0:
                    textFile = open("AI Score List.txt","a")
...
def nextSpace(line):
    ...
    global rotations , grid , shapes , mywindow , leftMostBlock, rightMostBlock , cItem , lowestBlock  , cLowest , leftMoves
    ...
    #else:
        #print("Cannot Place on line " + str(line))
    if leftMoves < 10:
        moveRight()
        leftMoves += 1
