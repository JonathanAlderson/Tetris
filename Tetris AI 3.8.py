    ...
    if spawnNewItem == True:
        ...
        for i in range(len(shapeToSpawn[0])):
            if myGrid[shapeToSpawn[0][i][1]][shapeToSpawn[0][i][0]] == 0:
                ...
                if playerScore > 0:
                    textFile = open("AI Score List 8.txt","a")
...
def nextSpace(line):
    ...
    if canPlace == True:
        for cube in cItem[0]:
            if cube[1]+line-cLowest+1 < 21:
                #print(" = " +
                #      str(grid[cube[1]+line-cLowest+1][cube[0]]))
                
                if grid[cube[1]+line-cLowest+1][cube[0]] == 0: #and [cube[1]+line-cLowest,cube[0]] not in [[cItem[0][0][1]+line-cLowest,cItem[0][0][0]],[cItem[0][1][1]+line-cLowest,cItem[0][1][0]],[cItem[0][2][1]+line-cLowest,cItem[0][2][0]],[cItem[0][3][1]+line-cLowest,cItem[0][3][0]]]:
                    numberOfBubbles += 1
                    #print("WOrked")
...
# 6 = T
# 7 = S
    if rotations < [2,4,4,0,2,4,2][cItem[1][0]-1]:
        rotations += 1
