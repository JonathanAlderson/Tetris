    ...
    if spawnNewItem == True:
        ...
        for i in range(len(shapeToSpawn[0])):
            if myGrid[shapeToSpawn[0][i][1]][shapeToSpawn[0][i][0]] == 0:
                ...
                if playerScore > 0:
                    textFile = open("AI Score List 4.txt","a")
...
def nextSpace(line):
    ...
    global rotations , grid , shapes , mywindow , leftMostBlock, rightMostBlock , cItem , lowestBlock  , cLowest , leftMoves , possibleAreasToPlace , numberOfBubbles
    ...
    if canPlace == True:
        for cube in cItem[0]:
            if cube[1]+line-cLowest+2 < 20:
                if grid[cube[1]+line-cLowest+2][cube[0]] == 0:
                    numberOfBubbles += 1
    ...
    if leftMoves == 10:
        if len(possibleAreasToPlace) > 0:
            #[leftMostBlock,numberOfBlocksBottomLayer,rotations,leftMoves,numberOfBubbles]
            #[[3, 2, 2, 3, 0], [4, 2, 2, 4, 0], [5, 2, 2, 5, 0], [6, 2, 2, 6, 0], [7, 2, 2, 7, 0], [7, 2, 2, 8, 0], [7, 2, 2, 9, 0], [7, 2, 2, 10, 0], [3, 2, 4, 3, 0], [4, 2, 4, 4, 0], [5, 2, 4, 5, 0], [6, 2, 4, 6, 0], [7, 2, 4, 7, 0], [7, 2, 4, 8, 0], [7, 2, 4, 9, 0], [7, 2, 4, 10, 0], [2, 1, 1, 2, 0], [2, 1, 3, 2, 0], [3, 1, 1, 3, 1], [4, 1, 1, 4, 1], [5, 1, 1, 5, 1], [6, 1, 1, 6, 1], [7, 1, 1, 7, 1], [8, 1, 1, 8, 1], [8, 1, 1, 9, 1], [8, 1, 1, 10, 1], [3, 1, 3, 3, 1], [4, 1, 3, 4, 1], [5, 1, 3, 5, 1], [6, 1, 3, 6, 1], [7, 1, 3, 7, 1], [8, 1, 3, 8, 1], [8, 1, 3, 9, 1], [8, 1, 3, 10, 1]]
